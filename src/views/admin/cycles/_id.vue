<template>
     ...
     <h1 class="text-3xl text-sky-400 text-center mb-2">Cycle Edit Page</h1>
...
export default {
    ...
    mounted() {
        ApiService.get("admin/cycles/"+this.id)
        .then((response) => {
    ...
    methods : {
        EditFunction() {
            ApiService.patch("admin/cycles/"+this.id,this.InputData)
            .then((response) => {
                if(response.data.condition){
                    this.$router.push("/admin/cycles");
                }else {
