<template>
    <div class="w-9/12 rounded mt-4 mx-auto py-4 bg-gray-800 text-gray-200/80">
     <h1 class="text-3xl text-sky-400 text-center mb-3">Attendance Edit Page</h1>
...
import ApiService from '../../../ApiService'
    export default {
        ...
        mounted () {
            ...
            ApiService.get("admin/attendances/"+this.id)
            .then((response) => {
                self.InputData = response.data.data
            })
        },
        methods : {
            EditFunction() {
                let self = this
                ApiService.patch("admin/attendances/"+this.id,this.InputData)
                .then((response) => {
