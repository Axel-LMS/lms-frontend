<template>
<div class="w-1/2 relative rounded mt-10 mx-auto py-4 bg-gray-800 text-gray-200/80">
                 <!-- image box -->
                 <div v-if="showImageBox" class="absolute w-full bg-gray-600 pb-8 border-2 border-gray-400/30 rounded top-0">
                <span @click="showImageBox = false"  class="block p-2 text-right cursor-pointer"><i class="fa-solid fa-x"></i></span>
    ...
    <form @submit.prevent="EditFunction" class="w-full justify-evenly flex flex-wrap">
        <div class="w-5/6">
          ...
          <div class="mb-4">
            ...
            <img v-if="previewImage" :src="previewImage" class="ml-6 rounded h-28 mt-6" alt="">
         </div>
         <div class="w-5/6 mb-24">
              <label class="block text-lg">Description</label>
              <quill-editor theme="snow" v-model:content="InputData.description" contentType="html"></quill-editor>
              ...
              <router-link :to="{name : 'Categories'}" class="text-center font-semibold text-gray-100 bg-sky-400 hover:bg-sky-900 w-11/12 mx-10 rounded-sm py-2 mb-2">Back</router-link>
        </form>
      </div>
</template>
...
import { QuillEditor } from '@vueup/vue-quill'
    ...
    export default {
        ...
        methods : {
            ...
            EditFunction() {
                ApiService.patch("admin/categories/"+this.id, this.InputData)
                .then((response) => {
