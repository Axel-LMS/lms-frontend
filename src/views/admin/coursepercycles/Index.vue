    ...
    <vue-good-table
      ...
      <template #table-row="props">
        <span class="flex justify-evenly" v-if="props.column.field == 'DE'">
            <router-link :to="{name : 'CoursepercycleEdit' , params : {id : props.row.id}}"><i class="fas fa-edit fa-lg mt-3 py-1"></i></router-link>
...
import { VueGoodTable } from 'vue-good-table-next';
    export default {
        ...
        methods : {
            deleteFun(param) {
                ApiService.delete("admin/coursepercycles/"+param)
                .then(() =>{
                    window.location.assign("/admin/coursepercycles");
                })
                .catch((error) => {
                    console.log(error);
                })
