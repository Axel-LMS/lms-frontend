 ...
 <h1 class="text-3xl text-sky-400 text-center mb-3">Course Edit Page</h1>
  <form @submit.prevent="EditFunction" class="w-full justify-evenly flex flex-wrap">
      ...
      <p class="text-rose-500 " v-if="failed.cycle_id">{{ failed.cycle_id[0] }}</p>
            ...
            <router-link :to="{name : 'Coursepercycles'}" class="text-center font-semibold text-gray-100 bg-sky-400 hover:bg-sky-900 w-11/12 mx-10 rounded-sm py-2 mb-2">Back</router-link>
      </form>
    </div>
</template>
...
methods : {
    EditFunction() {
        ApiService.patch("admin/coursepercycles/"+this.id,this.InputData)
        .then((response) => {
            if(response.data.condition){
                this.$router.push("/admin/coursepercycles");
            }else {
